...
Next, simply clone this repository to your home directory with: =git
clone https://github.com/Lewis-Kelley/ez-macs.git ~/.emacs.d=. If
there's already a directory there with the same name, delete it.
*** Linux
...
You don't have git installed. You really should, and I'd recommend
just installing it now. If you don't, insert the line =:disabled t= to
the [[#git-in-emacs-magit][Magit]] block below between =:ensure t= and =:init=.
...
#+BEGIN_SRC emacs-lisp
  (use-package magit
    ...
    :catch
    (lambda (keyword err)
      (message "You don't have git installed!")))
#+END_SRC
