...
** Installation
Firstly, follow the installation instructions for Emacs found
[[https://www.gnu.org/software/emacs/download.html][here]]. Then, install this config as described below. Launch the Emacs
binary you downloaded (found in =bin/emacs.exe=), and wait for the
initial setup to complete (it may take a moment). Subsequent launches
will take far less time.

*** Linux & Mac
Simply clone this repository to your home directory with the name
=~/.emacs.d=. Emacs tries to find this config file by looking for
=~/.emacs.d/init.el=, so just make sure that this is at your home
directory and with the correct name.
*** Windows
